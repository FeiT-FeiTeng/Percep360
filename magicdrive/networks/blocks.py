...
from magicdrive.networks.ff_attn import OS_SSM
from magicdrive.networks.gatecross import BGasicTransformerBlock
...
class BasicMultiviewTransformerBlock(BasicTransformerBlock):
    ...
    def __init__(
        ...
        # )
        self.attn4 = BGasicTransformerBlock(
            query_dim=dim,
            key_dim=dim,
            value_dim=dim,
    ...
    def forward(
        ...
        **kwargs,
        ...
        multi_attention = "self"
        if multi_attention == "2 Self":
            print("Cross_View_Attention")
            norm_hidden_states = (
            ...
            if kwargs.get("noisy_latents_box") is not None:
                attn_output = self.attn4(x = norm_hidden_states,context=norm_hidden_states,objs=None
                                     ,perl_box_masking_map=kwargs["noisy_latents_box"],perl_road_masking_map=kwargs["noisy_latents_depth"],road_map_embedding=None)
            else:
                attn_output = self.attn4(x = norm_hidden_states,context=norm_hidden_states,objs=None
                                     ,perl_box_masking_map=None,perl_road_masking_map=None,road_map_embedding=None)
        else:
            #print("With_out Attn")
            norm_hidden_states = (
                self.norm4(hidden_states, timestep) if self.use_ada_layer_norm else
