...
class BEVControlNetModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # depth = kwargs["noisy_latents2"].to(sample.device)
        # depth = rearrange(depth, 'b n ... -> (b n) ...')
        # depth = self.conv_in_d(depth)


        # 2. pre-process
        #controlnet_cond=torch.nn.functional.interpolate(controlnet_cond, size=(136, 1200), mode='bilinear', align_corners=True)

#        sample = self.depth_down(sample)
        sample = self.conv_in(sample)

        controlnet_cond = self.controlnet_cond_embedding(controlnet_cond) #torch.Size([3, 320, 28, 50])    torch.Size([1, 320, 20, 300])
        if 'flip' in kwargs and kwargs['flip'] is not None:
            #print("controlnet_cond,roll")
            B_dim_size = controlnet_cond.size(-1)
            B_shift = int(kwargs['flip'] * B_dim_size)
            controlnet_cond = torch.roll(controlnet_cond, shifts=B_shift, dims=-1)
            #print("flip")
        controlnet_cond = torch.nn.functional.interpolate(controlnet_cond,size=sample.shape[-2:],mode = 'bilinear',align_corners=True)
        # sample += depth  #torch.Size([8, 320, 10, 150]) torch.Size([8, 4, 10, 150])
        sample += controlnet_cond

        # 3. down
        down_block_res_samples = (sample,)
        for downsample_block in self.down_blocks:  ## Training = True
            if (
        ...
        # 4. mid
        if self.mid_block is not None: ###Training = True
