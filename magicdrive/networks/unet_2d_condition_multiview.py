...
class UNet2DConditionModelMultiview(UNet2DConditionModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.time_embed_act is not None:  #None
            emb = self.time_embed_act(emb)

        if self.encoder_hid_proj is not None:   #None
        ...
        for downsample_block in self.down_blocks:
            if self.crossview_attn_type == 'epipolar': #Basic
        ...
        if self.mid_block is not None:
            if self.crossview_attn_type == 'epipolar':
                cross_attention_kwargs['out_size'] = sample.shape[-2:]
            #kwargs.pop('bboxes_3d_data', None)
