...
class ControlnetUnetWrapper(ModelMixin):
    ...
    def forward(self, noisy_latents,noisy_latents_box,noisy_latents_depth, timesteps, camera_param,
                encoder_hidden_states, encoder_hidden_states_uncond,
                controlnet_image, **kwargs):
        N_cam = noisy_latents.shape[1]
        flip_ratio = kwargs.pop('flip', None)
        
        kwargs = move_to(
            kwargs, self.weight_dtype, lambda x: x.dtype == torch.float32)
        kwargs['flip'] = flip_ratio
        
        kwargs["noisy_latents_box"] = noisy_latents_box
        kwargs["noisy_latents_depth"] = noisy_latents_depth
        
        kwargs["noisy_latents_box"] = kwargs["noisy_latents_box"].reshape(
            -1,  # Infer B*N.
            kwargs["noisy_latents_box"].shape[2],  # Keep the channel dimension.
            kwargs["noisy_latents_box"].shape[3] * kwargs["noisy_latents_box"].shape[4]  # H*W
        )

        kwargs["noisy_latents_depth"] = kwargs["noisy_latents_depth"].reshape(
            -1,  # B*N
            kwargs["noisy_latents_depth"].shape[2],  # C
            kwargs["noisy_latents_depth"].shape[3] * kwargs["noisy_latents_depth"].shape[4]  # H*W
        )

        kwargs["noisy_latents_box"] = (kwargs["noisy_latents_box"]).sum(dim=1, keepdim=True) 
        kwargs["noisy_latents_depth"] = (kwargs["noisy_latents_depth"]).sum(dim=1, keepdim=True)   
...
class MultiviewRunner(BaseRunner):
    ...
    def _set_model_trainable_state(self, train=True):
        ...
        self.vae.requires_grad_(False)#(False)
    ...
    def _train_one_stop(self, batch):
        ...
        with self.accelerator.accumulate(self.controlnet_unet):
            ...
            fp = 1  # Previously sampled with torch.rand(1).item().
            if fp > 0.5:
                dim_size = batch["pixel_values"].size(-1)
                # Randomly sample a panorama roll offset.
                shift = random.randint(1, dim_size - 1)
                batch["pixel_values"] = torch.roll(batch["pixel_values"], shifts=shift, dims=-1)
                # Keep the normalized roll offset for downstream alignment.
                flip_ratio = shift / dim_size
                batch["box"] = torch.roll(batch["box"], shifts=shift, dims=-1)
                batch["depth"] = torch.roll(batch["depth"], shifts=shift, dims=-1)
            else:
                flip_ratio=None
            batch['kwargs']['flip'] = flip_ratio
            
            latents = self.vae.encode(
                rearrange(batch["pixel_values"], "b n c h w -> (b n) c h w").to(
                    dtype=self.weight_dtype
                )
            ).latent_dist.sample()  #torch.Size([8, 1, 3, 80, 1200]) --->  torch.Size([8, 4, 10, 150])

            latents_box = self.vae.encode(
                rearrange(batch["box"], "b n c h w -> (b n) c h w").to(
                    dtype=self.weight_dtype
                )).latent_dist.sample()
            
            latents_depth = self.vae.encode(
                rearrange(batch["depth"], "b n c h w -> (b n) c h w").to(
                    dtype=self.weight_dtype
                )).latent_dist.sample()

            latents = latents.to(dtype=self.weight_dtype)
            latents_box = latents_box.to(dtype=self.weight_dtype)
            latents_depth = latents_depth.to(dtype=self.weight_dtype)

            
            latents = latents * self.vae.config.scaling_factor
            latents = rearrange(latents, "(b n) c h w -> b n c h w", n=N_cam) #torch.Size([8, 1, 4, 10, 150])
            
            latents_box = latents_box * self.vae.config.scaling_factor
            latents_box = rearrange(latents_box, "(b n) c h w -> b n c h w", n=N_cam)  #torch.Size([8, 1, 4, 10, 150])    

            latents_depth = latents_depth * self.vae.config.scaling_factor
            latents_depth = rearrange(latents_depth, "(b n) c h w -> b n c h w", n=N_cam)  #torch.Size([8, 1, 4, 10, 150])      

            # embed camera params, in (B, 6, 3, 7), out (B, 6, 189)
            # camera_emb = self._embed_camera(batch["camera_param"])
            camera_param = batch["camera_param"].to(self.weight_dtype)               #torch.Size([1, 1, 3, 7])

            # Sample noise that we'll add to the latents
            noise = torch.randn_like(latents)                                       #torch.Size([1, 1, 4, 20, 300])
            ...
            noise_latents_box = latents_box

            noise_latents_depth = latents_depth

            # Get the text embedding for conditioning
            encoder_hidden_states = self.text_encoder(batch["input_ids"])[0]          #torch.Size([1, 24, 768])
            encoder_hidden_states_uncond = self.text_encoder(
                batch
                ["uncond_ids"])[0]                                                       #torch.Size([1, 24, 768])

            controlnet_image = batch["bev_map_with_aux"].to(                          #torch.Size([1, 8, 200, 200])
                dtype=self.weight_dtype)

            model_pred = self.controlnet_unet(
                noisy_latents,noise_latents_box,noise_latents_depth, timesteps, camera_param, encoder_hidden_states,
            ...
            loss = F.mse_loss(
                model_pred.float(), target.float(), reduction='none')   #model_pred:torch.Size([16, 1, 4, 10, 150])  target:torch.Size([16, 1, 4, 10, 150])
            loss = loss.mean()
