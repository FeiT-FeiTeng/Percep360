...
#####################################################3
    if "depth" in examples[0] and examples[0]["depth"]:
        # multi-view images
        examples[0]["depth"] = torch.stack(
            [example["depth"].data for example in examples])
        examples[0]["depth"] = examples[0]["depth"].to(
            memory_format=torch.contiguous_format).float()
        ret_dict["depth"] = examples[0]["depth"]
    elif is_train and examples[0]["depth"]:
        raise RuntimeError("For training, you should provide depth images.")
    if "box" in examples[0] and examples[0]["box"]:
        # multi-view images
        examples[0]["box"] = torch.stack(
            [example["box"].data for example in examples])
        examples[0]["box"] = examples[0]["box"].to(
            memory_format=torch.contiguous_format).float()
        ret_dict["box"] = examples[0]["box"]
    elif is_train and examples[0]["box"]:
        raise RuntimeError("For training, you should provide box images.")
