...
from params import save_trainable_params
...
def main(cfg: DictConfig):
    ...
    # datasets
    train_config_str = OmegaConf.to_container(cfg.dataset.data, resolve=True)
    with open('../train_dataset_config.txt', 'w') as f:
        f.write(str(train_config_str))
    
    train_dataset = build_dataset(
        OmegaConf.to_container(cfg.dataset.data.train, resolve=True)
    )
    val_dataset = build_dataset(
        OmegaConf.to_container(cfg.dataset.data.val, resolve=True)     #NuScenesDatasetM
    )
...
if __name__ == "__main__":
    import os
    os.environ.setdefault("PANODREAMER_PATH", os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
    main()
