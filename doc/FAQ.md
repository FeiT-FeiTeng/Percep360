...
We tested with `python=3.8`, `gcc-0.9.3`, `g++-0.9.3`, and `cuda-11.3`. The final environment snapshot is [requirements/a800.yaml](../requirements/a800.yaml).
...
This may appear when you install bevfusion (mmdet3d) on cuda10.2. The latest version of bevfusion supports Ampere GPUs by hard-coding compile parameters, leading to error when compiled with cuda10.2. If you build bevfusion from source, adjust the compile flags in its `setup.py`.
```python
...
Video generation is tracked separately from the `main` branch workflow.


### Q5: AttributeError: 'BasicTransformerBlock' object has no attribute '_args'

[#7](https://github.com/cure-lab/MagicDrive/issues/7).
Use the Diffusers version pinned for this project. See [requirements/dev.txt](../requirements/dev.txt) and [requirements/a800.yaml](../requirements/a800.yaml).
```bash
